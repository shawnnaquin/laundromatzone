                ...
                $('#themify_builder_site_canvas_iframe').one('load', function () {
                    ...
                    $body.one('themify_builder_ready', function (e) {
                        ...
                        for (var i = 0, len = css.length; i < len; ++i) {
                            if (css[i] && css[i].parentNode && css[i].getAttribute( 'href' ).search( /(?=wp\-includes\/(css|js).(?!admin\-bar)).+/ ) === -1 ) {
                        ...
                        $body.prop('class', 'themify_builder_active builder-breakpoint-desktop'+($body.hasClass('tb_module_panel_locked')?' tb_module_panel_locked':'')).removeAttr('style');
                        tbLoaderVars = null;
