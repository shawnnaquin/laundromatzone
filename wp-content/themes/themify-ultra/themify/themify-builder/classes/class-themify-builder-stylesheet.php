...
class Themify_Builder_Stylesheet {
    ...
    private function get_custom_styling($style_id, $mod_name, $settings, $format = 'tag') {
        ...
        foreach ($settings as $id => $value) {
            ...
            if (!empty($rules[$mod_name][$id]) || $is_gradient || $is_cover) {
                ...
                if (!empty($selector) && !empty($opt['selector'])) {
					...
					$css[$opt['selector']] = $css[$opt['selector']];
                }
            }
        }
        $output = array();
        if (!empty($css)) {
            foreach ($css as $selector => $defs) {
                ...
                $output[] = implode(',', $styles) . '{' . Themify_Builder_Stylesheet::filter_css_props( $defs ) . '}' . "\n";
            }
        }
        if ('tag' === $format) {
                            ...
                            if (!empty($lp_meta)) {
                                ...
                                foreach ($lp_meta as $lp_row_index => $lp_row) {
                                    if (!empty($lp_row['styling']) && is_array($lp_row['styling'])) {
                                         $css_to_save.= $this->get_custom_styling(".themify_builder_{$lp->ID}_row.module_row_{$lp_row['row_order']}", 'row', $lp_row['styling'], 'css');
                                    }
	...
	public static function filter_css_props( $css ) {
		$filtered_css = array();
		$css = array_filter( array_map( 'trim', explode( ';', $css ) ) );
		
		foreach( $css as $prop ) {
			preg_match( "/^[^:]+/", $prop, $p );

			if( ! empty( $p[0] ) ) {
				$filtered_css[ $p[0] ] = $prop;
			}
		}
